...
export class WeatherService {
  ...
  getAPIByCity (city: string): Observable<WeatherModel[]> {
    ...
    return this.http.get<WeatherModel[]>(this._urlWeather + isCity).catch(this.errorHandle);
  }

  getAPIByForecast (id: string): Observable<WeatherModel[]> {
    const isId = (id) ? id : this._idDefault;
    console.log(this._urlForecast + isId)
    return this.http.get<WeatherModel>(this._urlForecast + isId).catch(this.errorHandle);
  }
